...
| --- | --- | --- |
| `Flow(ocp, u, g, μ)` (3 positional) | `PreconditionError`, thrown by CTFlows (not by `OptimalControl`), naming the keyword form: `Flow(ocp, u; constraint=g, multiplier=μ)` | A 4-arity shim here would overwrite CTFlows' own method for this exact signature — and it need not, CTFlows guards it directly ([CTFlows#401](https://github.com/control-toolbox/CTFlows.jl/issues/401), fixed). |
...
| `CTSolvers.Modelers.ADNLP()`, `CTDirect.Collocation()` | `UndefVarError` on `CTSolvers` / `CTDirect` — the module name is not bound under `using OptimalControl` | `CTSolvers`, `CTDirect` and `CTParser` are implementation packages and are deliberately **not** re-exported (unlike `CTBase`, `CTModels`, `CTFlows`, `CTLie`), so there is no bound name to attach a throwing method to. The constructors themselves *are* re-exported: write `OptimalControl.ADNLP()` / `OptimalControl.Collocation()`. |
