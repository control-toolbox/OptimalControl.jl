...
function Flow(vf::VectorField, description...; kwargs_Flow...)
   ...
   TBW
   """
    function f(tspan::Tuple{Time,Time}, x0::State, λ...; method=__method(), abstol=__abstol(), reltol=__reltol(), saveat=__saveat(), kwargs...)
        if isempty(λ)
   ...
   TBW
   """
    function f(t0::Time, x0::State, t::Time, λ...; method=__method(), abstol=__abstol(), reltol=__reltol(), saveat=__saveat(), kwargs...)
        sol = f((t0, t), x0, λ..., method=method, abstol=abstol, reltol=reltol, saveat=saveat; kwargs..., kwargs_Flow...)
        n = size(x0, 1)
        return sol[1:n, end]
    end
