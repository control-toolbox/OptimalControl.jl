...
f(x) = (1 / 2) * norm(x)^2
g(x) = x
dp = OptimalControl.DescentProblem(f, g)
x0 = [1.0; 0.0]
di = OptimalControl.DescentInit(x0)
s = [0.0; 0.0]

@test typeof(dp) == OptimalControl.DescentProblem
@test typeof(di) == OptimalControl.DescentInit

ds = OptimalControl.descent_solver(dp, di, display=false)
@test ds.x ≈ s atol = 1e-8

ds = OptimalControl.descent_solver(dp, di, direction=:gradient, line_search=:backtracking, display=false)
@test ds.x ≈ s atol = 1e-8

ds = OptimalControl.descent_solver(dp, di, direction=:gradient, line_search=:bissection, display=false)
@test ds.x ≈ s atol = 1e-8

ds = OptimalControl.descent_solver(dp, di, direction=:gradient, line_search=:fixedstep,
      optimalityTolerance=1e-3, display=true)
@test ds.x ≈ s atol = 1e-3

ds = OptimalControl.descent_solver(dp, di, direction=:bfgs, line_search=:backtracking, display=false)
@test ds.x ≈ s atol = 1e-8

ds = OptimalControl.descent_solver(dp, di, direction=:bfgs, line_search=:bissection, display=false)
@test ds.x ≈ s atol = 1e-8

ds = OptimalControl.descent_solver(dp, di, direction=:bfgs, line_search=:fixedstep,
      optimalityTolerance=1e-3, display=true)
@test ds.x ≈ s atol = 1e-3

# --------------------------------------------------------------------------------------------------
#
direction, line_search = OptimalControl.read((:gradient, :bissection, :tata))
...
#
# initialization
ocp_c = OptimalControl.convert(ocp, RegularOCPFinalConstraint)
ocp2descent_init = OptimalControl.ocp2descent_init
__grid_size = OptimalControl.__grid_size
__init = OptimalControl.__init
__grid = OptimalControl.__grid
__init_interpolation = OptimalControl.__init_interpolation
...
@test init_.x == OptimalControl.DescentInit(__init(ocp_c)).x
...
N = length(grid)
@test init_.x == OptimalControl.DescentInit(__init(ocp_c, N)).x
@test grid_ == grid
...
@test init_.x ≈ OptimalControl.DescentInit([[u_sol(grid_[i])-1.0] for i = 1:__grid_size()-1]).x atol=1e-4
@test grid_ == __grid(ocp_c)
...
@test init_.x == OptimalControl.DescentInit(U).x
@test grid_ == T
...
@test init_.x ≈ OptimalControl.DescentInit([[u_sol(grid_[i])-1.0] for i = 1:N2-1]).x atol=1e-4
@test grid_ == T2
# T1
...
@test init_.x ≈ OptimalControl.DescentInit([[u_sol(T[i])-1.0] for i = 1:N-1]).x atol=1e-4
@test grid_ == T
