...
f(x) = (1 / 2) * norm(x)^2
g(x) = x
dp = DescentProblem(f, g)
x0 = [1.0; 0.0]
di = DescentInit(x0)
s = [0.0; 0.0]

@test typeof(dp) == DescentProblem
@test typeof(di) == DescentInit

ds = descent_solver(dp, di, display=true)
@test ds.x ≈ s atol = 1e-8

ds = descent_solver(dp, di, direction=:gradient, line_search=:backtracking, display=false)
@test ds.x ≈ s atol = 1e-8

ds = descent_solver(dp, di, direction=:gradient, line_search=:bissection, display=false)
@test ds.x ≈ s atol = 1e-8

ds = descent_solver(dp, di, direction=:gradient, line_search=:fixedstep,
      optimalityTolerance=1e-3, display=false)
@test ds.x ≈ s atol = 1e-3

ds = descent_solver(dp, di, direction=:bfgs, line_search=:backtracking, display=false)
@test ds.x ≈ s atol = 1e-8

ds = descent_solver(dp, di, direction=:bfgs, line_search=:bissection, display=false)
@test ds.x ≈ s atol = 1e-8

ds = descent_solver(dp, di, direction=:bfgs, line_search=:fixedstep,
      optimalityTolerance=1e-3, display=false)
@test ds.x ≈ s atol = 1e-3
...
#
# initialization
ocp_c = convert(ocp, RegularOCPFinalConstraint)
...
@test init_.x == DescentInit(__init(ocp_c)).x
...
N = length(grid)
@test init_.x == DescentInit(__init(ocp_c, N)).x
@test grid_ == grid
...
@test init_.x ≈ DescentInit([[u_sol(grid_[i])-1.0] for i = 1:__grid_size()-1]).x atol=1e-4
@test grid_ == __grid(ocp_c)
...
@test init_.x ≈ DescentInit([[u_sol(grid_[i])-1.0] for i = 1:N2-1]).x atol=1e-4
@test grid_ == T2
# T1
...
@test init_.x ≈ DescentInit([[u_sol(T[i])-1.0] for i = 1:N-1]).x atol=1e-4
@test grid_ == T
